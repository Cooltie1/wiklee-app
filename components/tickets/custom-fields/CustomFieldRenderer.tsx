  ...
  return (
    <div className={isBooleanField ? "space-y-1" : "space-y-2"}>
      ...
      {definition.field_type === "boolean" && (
        <div className={`flex min-h-10 items-center rounded-md border border-zinc-200 px-3 py-2 ${textFieldClassName ?? "bg-white"}`}>
          <label htmlFor={definition.id} className="flex w-full items-center gap-3 text-sm text-zinc-700">
            <Checkbox
              id={definition.id}
              checked={value === true}
