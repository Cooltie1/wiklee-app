    ...
    <Popover>
            ...
            {selectedOptions.length ? (
              <div className="flex max-w-full flex-wrap gap-1.5 whitespace-normal">
                {selectedOptions.map((option) => (
                  <span
                    key={option.value}
                    className="max-w-full rounded-md bg-zinc-900 px-2 py-0.5 text-[11px] leading-4 text-white break-words"
                  >
                    {option.label}
                  </span>
                ))}
      ...
      >
        <div className="space-y-2">
          {options.length === 0 ? (
            <p className="text-xs text-zinc-500">No options configured.</p>
          ) : null}
          {options.map((option) => {
  ...
  return (
    <div className={isBooleanField ? "space-y-1" : "space-y-2"}>
      {!isBooleanField ? (
        ...
        >{`${definition.label}${requiredMark}`}</Label>
      ) : null}
